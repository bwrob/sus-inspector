...
def test_remove_injection(tmp_path: Path) -> None:
    ...
    # Test error on open for read
    with patch.object(Path, "open", side_effect=OSError("Fail")):
        with patch(
            "sus_inspector.cli.get_target_info", return_value=(tmp_path, "test.py")
        ):
            # It only raises SystemExit if it fails on WRITE, not on read (read handles it via return)
            # Actually, the current code doesn't catch read errors in remove_injection.
            # Let's check the code: it doesn't have try/except around open(encoding="utf-8") as f: f.readlines()
            pass
    ...
    def mocked_open(self: Path, *args: Any, **kwargs: Any) -> Any:  # noqa: ANN401
        if self == f and args and args[0] == "w":
            raise OSError("Write fail")
        return orig_open(self, *args, **kwargs)

    with patch.object(Path, "open", side_effect=mocked_open, autospec=True):
        with patch(
            "sus_inspector.cli.get_target_info", return_value=(tmp_path, "test.py")
        ):
            with pytest.raises(SystemExit):
